...
class CategoryDetailVC: UIViewController {
    ...
    //Constants
    struct Constants{
        static let CategoryDetailViewCell = "CategoryDetailViewCell"
    }
    ...
    var loadControl: Int = 0
    var actionSheetControl: Int = 0
    ...
    @IBAction func filterResults(_ sender: AnyObject) { //arrange by week
        ...
        if sender.selectedSegmentIndex == 1{ //arrange by rank
            print ("shit")
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (loadControl == 0){
            ...
            if categorycode.isEmpty {
                print ("loadAllCategory()")
                loadAllCategory()
            } else {
                print ("loadByCategory")
                print(categorycode)
    ...
    func registerNIBs(){
        tableView.register(UINib(nibName: Constants.CategoryDetailViewCell, bundle: nil), forCellReuseIdentifier: Constants.CategoryDetailViewCell)
    }
    ...
    func loadByCategory(category: String) {
        ...
        if hasConnectivity() {
            ...
            let parameters: Parameters = ["api-key":"7d74be64d0cb4324a9cf8e349d00c6c1"]
    ...
    func loadAllCategory() {
        ...
        if hasConnectivity() {
            let parameters: Parameters = ["published_date": "\(date)",
                "api-key":"7d74be64d0cb4324a9cf8e349d00c6c1"]
    ...
    func addSortingButton() {
        ...
        let refreshButton = UIBarButtonItem(title: "Filter", style: UIBarButtonItemStyle.done, target: self, action: #selector(CategoryDetailVC.buttonMethod))
    ...
    func buttonMethod() {
        ...
        // Show
        if (actionSheetControl == 0){
            let actionSheet = CustomizableActionSheet()
            actionSheet.showInView(self.view, items: items)
            actionSheetControl += 1
        }
    }
...
extension CategoryDetailVC:UITableViewDataSource,UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell:CategoryDetailViewCell = self.tableView.dequeueReusableCell(withIdentifier: Constants.CategoryDetailViewCell) as! CategoryDetailViewCell
        /*
        ...
        //cell.bookTitleLabelHieght.constant = heightForView(text: "\(bookTitleArray[indexPath.row])", font: UIFont(name: "Times New Roman", size: 17.5)!, width: self.view.bounds.size.width - 166.0)
        //cell.bookAuthorLabelHeight.constant = heightForView(text: "by \(bookAuthorArray[indexPath.row]) \nin \(categoryName)", font: UIFont(name: "Times New Roman", size: 13.5)!, width: self.view.bounds.size.width - 166.0)
         */
        ...
        cell.bookDescription.text = self.bookDescriptionArray[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let detailView:BookDetailVC = self.storyboard?.instantiateViewController(withIdentifier: "BookDetailVC") as! BookDetailVC
